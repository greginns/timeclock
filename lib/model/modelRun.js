...
const {SQLInsertError, SQLUpdateError, SQLSelectError, SQLDeleteError, SQLSchemaError, UserError} = require(root + '/lib/errors.js');
      ...
      if (Object.keys(obj).length > 0) {
        ...
        Object.keys(obj).forEach(function(col) {
          if (cols.indexOf(col) != -1 || overRideColumns) {
            self[col] = (obj[col] || obj[col] === false) ? obj[col] : ('default' in schema[col].defn) ? schema[col].defn.default : obj[col];
          }
        })
      }
      else {
...
/* Instance actions */  
  async insertOne({pgschema = ''} = {}) {
    ...
    if (!tm.err) {
      if (tm.data.length == 0) {
        tm.err = new SQLInsertError(text);
      }
      else {
  ...
  async updateOne({pgschema = ''} = {}) {
    ...
    if (!tm.err) {
      if (tm.data.length == 0) {
        tm.err = new SQLUpdateError(text);
      }
      else {
        //tm.data = tcon.construct(tm.data)[0];
        tm.data = tm.data[0];
...
/* Model actions */  
  ...
  static async select({pgschema = '', rec = {}, cols = '*', showHidden = false} = {}) {
    ...
    [where, values] = tcon.makeSelectValues(rec);
    text = `SELECT ${colNames} FROM ${table}` + ((where.length > 0) ? ` WHERE ${where.join(' AND ')}` : '');
    text += ' ' + orderBy;
    tm = await tcon.sql({text, values});
  ...
  static async insert({pgschema='', rows=[]} = {}) {
    ...
    for (row of rows) {
      ...
      if (tm.err) {
        ...
        if (tm.data.length == 0) {
          tmx.data.push({isError: true, error: (new SQLInsertError()).message});
        }
        else {
