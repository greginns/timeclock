App.mvcObjs.tenant_manage_users = {
  model: {
    user: {},
    userOrig: {},
    userPK: '',
  ...
  lifecycle: {
    created: function() {
      ...
      App.subs.data.subscribe('/tenant/user', function(data) {
        self.$set('users', data);
      })
    ...
    inView: function(params) {
      ...
      $('#tenant-manage-navbar-users').addClass('active disabled');
      $('#tenant-users-toast1').toast('hide');
    },

    outView: function() {
      $('#tenant-manage-navbar-users').removeClass('active disabled');
    }
  },
    ...
    save: async function(ev) {
      ...
      .then(function(res) {
        if (res.status == 200) {
          ...
          $('#tenant-manage-users-toast1').toast('show');
  ...
  watch: {
    'user.code': function(nv) {
      if (nv) {
        if (Object.keys(this.getUserFromList(nv)).length > 0) {
          this.$set('userOrig.code', nv);
          this.$set('userPK', nv);
        }
      }
    },
